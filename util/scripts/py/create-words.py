...
def create_textgrid(aramaic_sentence, inter_sentence, audio_file_size, output_filename):
    # Split the sentence into words
    words = aramaic_sentence.split(" ")
    inters = inter_sentence.split("\t")
    ...
    # Add the "Words" tier
    words_tier = textgrid.IntervalTier(
        name="Words", minTime=0, maxTime=audio_file_size)
    ...
    # Add the "Inter" tier
    inter_tier = textgrid.IntervalTier(
        name="Inter", minTime=0, maxTime=audio_file_size)
    for i in range(len(words)):
        start_time = i * interval_duration
        end_time = (i + 1) * interval_duration
        inter_tier.add(start_time, end_time, inters[i])
    tg.append(inter_tier)

    # Add the "Segment" tier
    segment_tier = textgrid.IntervalTier(
        name="Segment", minTime=0, maxTime=audio_file_size)
...
if __name__ == '__main__':
    ...
    # Get the "VerseNo" tier
    inter_tier = tg.getFirst('VerseNo')
    inter_interval = inter_tier[verse_number]

    aramaic_sentence = verse_interval.mark
    inter_sentence = inter_interval.mark
    audio_length = get_audio_length('wav/' + input_filename + 'wav')
    output_filename = 'wav/' + input_filename + 'TextGrid'

    textgrid_content = create_textgrid(
        aramaic_sentence, inter_sentence, audio_length, output_filename)
